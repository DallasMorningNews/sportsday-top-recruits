...
// ----- LEAFLET SETUP -----
	...
	*/
	...
	$("#mapPosition").change(function() {
		$("#mapPosition option:selected").each(function() {
			...
			map.removeLayer(allRecruitsMarkers);
			map.removeLayer(qb_Markers);
			map.removeLayer(rb_Markers);
			map.removeLayer(wr_Markers);
			map.removeLayer(te_Markers);
			map.removeLayer(ol_Markers);
			map.removeLayer(dl_Markers);
			map.removeLayer(lb_Markers);
			map.removeLayer(db_Markers);
			map.removeLayer(ath_Markers);
			map.removeLayer(k_Markers);
			addPositionLayers(targetPosition);;
		})
	})
	...
	*/
	...
	function tableBuilder(school, position) {
		...
		var count = $("#recruitsTable table td").length;
		if (count === 0) {
	...
	*/
	...
	--------------------------
	*/
	$('.leaflet-clickable').addClass('circleFade');
